...
- [x] **[verified]** `components/about/person-card.tsx` — founder badge reads `"Youtube Face"`. → `Founder`. **Marked done on 2026-07-30 without being done; the string was still live on `/about`.** Genuinely fixed 2026-07-31 and confirmed absent from the rendered HTML. `person.role` still renders the real role beside it rather than being replaced by the badge.
